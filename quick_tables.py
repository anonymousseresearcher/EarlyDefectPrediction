...
def getMetricMedian(measuresCSVPath, metric, policy):
    ...
    if len(medianValues) == 1:
        ...
        if  rankValues[0] >=  df['rank'].max()     and metric in ['roc_auc', 'recall' , 'g-score']:
...
def generateTable(measuresCSVPath, fileName, sortedMap):
    ...
    for key, v in sortedMap.items():
        ...
        mv, win = getMetricMedian(measuresCSVPath,'g-score', key)
        f += win
        gscore.append(mv)
    ...
    df.to_csv('quick_'+fileName+'.csv', index=False)
    print("Generated ",' quick_'+fileName+'.csv in the current working directory!')
...
def run(fileName, measuresCSVPath):
        ...
        gscoreDF = pd.read_csv(measuresCSVPath+'zg-score.csv')
...
if __name__ == '__main__':
    ...
    print("Generating")
    run('TableIV', './results/table_1/')
    run('TableV', './results/table_2/')
